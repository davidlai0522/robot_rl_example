...
class TowerOfHanoi(ManipulationEnv):
    ...
    def _reset_internal(self):
        ...
        for obj_pos, obj_quat, obj in {
            "pole_0": (np.array([0.2, -0.35, self.disk_height/2]), np.array([0, 0, 0, 1]), self.poles[0]),
            "pole_1": (np.array([0.2, -0.35, self.disk_height/2 + self.disk_height]), np.array([0, 0, 0, 1]), self.poles[1]),
            "pole_2": (np.array([0.2, -0.35, self.disk_height/2 + 2*self.disk_height]), np.array([0, 0, 0, 1]), self.poles[2]),
        }.values():
        ...
        # Loop through all objects and reset their positions
        for obj_pos, obj_quat, obj in {
            "disk_0": (np.array([0.2, -0.35, self.disk_height/2]), np.array([0, 0, 0, 1]), self.disks[0]),
            "disk_1": (np.array([0.2, -0.35, self.disk_height/2 + self.disk_height]), np.array([0, 0, 0, 1]), self.disks[1]),
            "disk_2": (np.array([0.2, -0.35, self.disk_height/2 + 2*self.disk_height]), np.array([0, 0, 0, 1]), self.disks[2]),
        }.values():
            self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array(obj_pos), np.array(obj_quat)]))
